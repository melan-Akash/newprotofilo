    ...
    return (
        <div id="experience" className="w-full px-[12%] py-10 scroll-mt-20">
            ...
            <div className="max-w-4xl mx-auto relative">
                {/* Timeline line */}
                <div className="hidden sm:block absolute left-8 top-0 bottom-0 w-px bg-gradient-to-b from-sky-500 to-blue-600 dark:from-sky-400 dark:to-blue-500"></div>

                <div className="flex flex-col gap-10">
                    {experiences.map((exp) => (
                        <div key={exp.role} className="sm:pl-20 relative group">
                            {/* Timeline dot */}
                            <div className="hidden sm:flex absolute left-[22px] top-6 w-[13px] h-[13px] rounded-full bg-gradient-to-r from-sky-500 to-blue-600 border-[3px] border-white dark:border-darkTheme z-10 group-hover:scale-125 transition-transform duration-300"></div>

                            <div className="border border-gray-300 dark:border-white/20 rounded-xl p-6 sm:p-8 hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black dark:hover:shadow-white/80 dark:hover:bg-darkHover/50">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-4">
                                    <h3 className="text-lg font-semibold text-gray-800 dark:text-white">{exp.role}</h3>
                                    <span className="text-xs font-medium px-3 py-1 rounded-full bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-300 w-fit">{exp.type}</span>
                                </div>
                                ...
                                <ul className="space-y-2">
                                    {exp.points.map((point, i) => (
                                        <li key={i} className="text-sm text-gray-600 dark:text-white/75 leading-relaxed flex gap-2">
